...
//                FrameLayout.LayoutParams cameraPreviewParams = new FrameLayout.LayoutParams(300, 400);
                ((ViewGroup) webView.getView().getParent()).addView(mBarcodeView, cameraPreviewParams);

                Log.d("view", webView.getView().getClass().getName());

                ViewGroup viewGroup = (ViewGroup)webView.getView().getParent();
                for (int i =0;i < viewGroup.getChildCount(); i++) {
                    View child = viewGroup.getChildAt(i);
                    Log.d("child", child.getClass().getName());
                }
